...
function isMetadataFalse(value: unknown): boolean {
...
export function hasCompletedUsernameSetup(
  ...
  return isMetadataTrue(meta?.username_chosen);
}

/** Usernames auto-assigned by handle_new_user() before deferred onboarding. */
export function isAutoGeneratedUsername(raw: string): boolean {
  const username = normalizeUsername(raw);
  return /^birder(_[a-f0-9]{4})?$/i.test(username);
}
...
export async function resolveUsernameSetup(
  ...
  if (isMetadataTrue(meta.username_chosen)) return false;
  if (isMetadataFalse(meta.username_chosen)) return true;
  ...
  if (error) {
    ...
    // Fail closed: without a confirmed profile, send them to onboarding.
    return true;
  }
  ...
  const existing =
    typeof profile.username === "string" ? normalizeUsername(profile.username) : "";
  if (existing.length >= 3 && !isAutoGeneratedUsername(existing)) {
    return false;
  }
